...
{
	#[inline(always)]
	fn allocate(number_of_entries: usize) -> Self
	{
		let global = Global;
		let allocation = global::alloc(Self::layout(number_of_entries));
	...
	#[inline(always)]
	fn iterate<F: FnMut(*mut Entry<T>)>(self, number_of_entries: usize, callback: F)
	{
		...
		{
			unsafe { (entry_pointer_usize as *mut Entry<T>).write(Entry::new(index)) };
	...
	{
		let global = Global;
		global.dealloc(Self::layout(number_of_entries), self.inclusive_start_pointer() as *mut Opaque)
	}
	...
	#[inline(always)]
	fn entry_mutable_reference(self, index: usize) -> &mut Entry<T>
	{
		unsafe { &mut * self.entry_mutable_pointer(allocation, index) }
	}
	...
	#[inline(always)]
	fn pointer_usize_is_one_of_ours(self, pointer_usize: usize) -> bool
	{
		self.inclusive_start_pointer_usize() <= pointer_usize && pointer_usize < self.exclusive_end_pointer_usize()
	}

...
#[derive(Debug)]
pub(crate) struct SlabAllocation<T: Sized>
{
...
impl<T: Sized> Drop for SlabAllocation<T>
{
	#[inline(always)]
	fn drop(&mut self)
	{
		self.allocation.iterate(number_of_entries, |entry_pointer| unsafe { entry_pointer.drop_in_place() });
		
		self.allocation.deallocate(self.number_of_entries)
	}
}

impl<T: Sized> SlabAllocation<T>
{
	#[inline(always)]
	...
	#[inline(always)]
	pub(crate) fn new(number_of_entries: usize) -> Self
	{
	...
	#[inline(always)]
	pub(crate) fn allocate_unchecked(&mut self, value: T) -> &mut T
	{
	...
	#[inline(always)]
	pub(crate) fn free_unchecked(&mut self, entry_value: &mut T)
	{
		// NOTE: Works because entry.entry_value is the FIRST field in the Entry struct.
		let entry_pointer_usize = (entry_value as *mut T as usize);
		debug_assert!(self.allocation.pointer_usize_is_one_of_ours(entry_pointer_usize), "entry_value was not from this SlabAllocation");
	...
	{
		NonNull::<Entry<T>>::layout(number_of_entries)
	}
	
	#[inline(always)]
	fn size_in_bytes(number_of_entries: usize) -> usize
	{
		NonNull::<Entry<T>>::size_in_bytes(number_of_entries)
	}
	
	#[inline(always)]
	fn entry_size() -> usize
	{
		NonNull::<Entry<T>>::entry_size()
	}
}

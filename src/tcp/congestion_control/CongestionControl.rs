...
impl CongestionControl
{
	...
	pub(crate) fn new(initial_congestion_window_algorithm: InitialCongestionWindowAlgorithm, last_acknowledgment_occurred_at: MonotonicMillisecondTimestamp, sender_maximum_segment_size: u16, cached_congestion_data: &CachedCongestionData) -> Self
	{
		...
		Self
		{
			...
			last_acknowledgment_occurred_at,
			bytes_acked: 0,
			cwnd: IW,
			FlightSize: 0,
...
//		};
	...
	#[inline(always)]
	pub(crate) fn maximum_data(&self, SND: &TransmissionControlBlockSend) -> u32
	{
		let maximum_sequence_number = self.maximum_sequence_number(SND);
		if maximum_sequence_number > SND.NXT
		{
			maximum_sequence_number - SND.NXT
		} else {
			0
		}
	}
	...
	#[inline(always)]
	pub(crate) fn maximum_sequence_number(&self, SND: &TransmissionControlBlockSend) -> WrappingSequenceNumber
	{
		// RFC 5681 Section 2: "RECEIVER WINDOW (rwnd): The most recently advertised receiver window".
		let rwnd: u32 = SND.WND.into();
		(SND.UNA - 1) + min(self.congestion_window(), rwnd)
	}
	...
	{
		if (self.last_acknowledgment_occurred_at - now) > retransmission_time_out
		{

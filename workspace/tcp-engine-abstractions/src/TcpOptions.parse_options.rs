...
{
	($interface: ident, $packet: ident, $pointer_to_option_kind: ident, $end_pointer: ident) =>
	{
		{
	...
	{
		{
			...
			parse_option_variable_length_including_option_kind_and_length_fields!($interface, $packet, $pointer_to_option_kind, $end_pointer) as usize
		}
	}
}
...
{
	($interface: ident, $packet: ident, $pointer_to_block: ident) =>
	{
		{
			...
			while pointer_to_option_kind != end_pointer
			{
				let length = match TcpOptions::parse_option_kind_without_checks(pointer_to_option_kind)
				{
					...
					SelectiveAcknowledgmentOption::SelectiveAcknowledgmentPermittedOptionKind =>
					{
						...
						parse_option_known_length!($interface, $packet, pointer_to_option_kind, end_pointer, KnownLength);
					...
					SelectiveAcknowledgmentOption::Kind =>
					{
						...
						tcp_options.selective_acknowledgment = match length
						{
							...
							{
								let first_block = parse_selective_acknowledgment_block!($interface, $packet, pointer_to_first_block);
							...
							SelectiveAcknowledgmentOption::TwoBlocksLength =>
							{
								...
								let second_block = parse_selective_acknowledgment_block!($interface, $packet, pointer_to_second_block);
							...
							SelectiveAcknowledgmentOption::ThreeBlocksLength =>
							{
								...
								let third_block = parse_selective_acknowledgment_block!($interface, $packet, pointer_to_third_block);
							...
							SelectiveAcknowledgmentOption::FourBlocksLength =>
							{
								...
								let fourth_block = parse_selective_acknowledgment_block!($interface, $packet, pointer_to_fourth_block);
								Some(SelectiveAcknowledgmentOption::four_blocks(first_block, second_block, third_block, fourth_block))
							}
							
							_ => drop!($interface, $packet, "TCP option selective acknowledgment had more than 4 blocks (this error should not occur if option length is being properly validated)"),
						};
					...
					27 =>
					{
						...
						drop!($interface, $packet, "TCP option Quick-Start Response should never be received as we never initiate Quick-Start or respond passively to Quick-Start Request")
					}
					...
					UserTimeOutOption::Kind =>
					{
						...
						match length
						{
							...
							KnownUnofficialAkamaiLength =>
							{
								...
								{
									drop!($interface, $packet, "TCP option 28 (Akamai unofficial squatting) was duplicated")
								}
							...
							_ => drop!($interface, $packet, "TCP option 28 was neither user time out nor Akamai unofficial squatting")
						}
					...
					AuthenticationOption::Kind =>
					{
						...
						let length = parse_option_variable_length_including_option_kind_and_length_fields!($interface, $packet, pointer_to_option_kind, end_pointer);
					...
					30 => parse_unsupported_or_unknown_option!($interface, $packet, pointer_to_option_kind, end_pointer, duplicate_unknown_options, 30),
					...
					34 => parse_unsupported_or_unknown_option!($interface, $packet, pointer_to_option_kind, end_pointer, duplicate_unknown_options, 34),
